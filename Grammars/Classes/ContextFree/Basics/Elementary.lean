...
lemma language_of_cfgEmptyWord : CFgrammar.Language (@cfgEmptyWord T) = singleton [] :=
  ...
  change
    CFDerives (@cfgEmptyWord T) [Symbol.nonterminal (@cfgEmptyLang T).initial]
      (List.map Symbol.terminal w) at hw 
  cases
    @CF_tran_or_id_of_deri T (@cfgEmptyWord T) [Symbol.nonterminal cfg_empty_lang.initial]
  ...
  cases @CF_tran_or_id_of_deri T (@cfgEmptyWord T) List.nil (List.map Symbol.terminal w) step_none
  · by_contra contra
