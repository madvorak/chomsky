...
lemma CSgrammar.eq_or_tranDeri_of_deri {u w : List (Symbol T g.nt)} (ass : g.Derives u w) :
  u = w  ∨  ∃ v : List (Symbol T g.nt), g.Transforms u v ∧ g.Derives v w  :=
Relation.ReflTransGen.cases_head ass

lemma CSgrammar.eq_or_deriTran_of_deri {u w : List (Symbol T g.nt)} (ass : g.Derives u w) :
