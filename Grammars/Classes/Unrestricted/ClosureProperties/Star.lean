...
by
  ...
  apply grammar_deri_with_postfix
  rw [wlk_succ, List.take_succ, List.map_append, List.join_append, List.append_assoc,
    List.append_assoc]
  apply grammar_deri_with_prefix
...
private lemma cases_1_and_2_and_3a_match_aux {g : Grammar T} {r₀ : Grule T g.Nt}
    ...
    ∃ m : ℕ,
      ∃ u₁ v₁ : List (Symbol T g.Nt),
        u =
            List.join (List.map (· ++ [h]) (List.take m (List.map (List.map wrapSym) x))) ++
              List.map wrapSym u₁ ∧
          List.get? x m =
  ...
  have ul_lt :
    u.length < List.length (List.join (List.map (· ++ [H]) (List.map (List.map wrap_sym) x))) :=
    by
    clear * - lens
    linarith
  ...
  have hyppp :
    ...
    exact xxx.symm
  clear * - hyppp mm
  rw [List.map_append_append, List.map_append_append, List.join_append_append, List.append_assoc,
    ...
    List.append_left_inj, List.map_singleton, List.map_singleton, List.join_singleton,
...
private lemma case_1_match_rule {g : Grammar T} {r₀ : Grule T g.Nt}
    ...
    ∃ m : ℕ,
      ∃ u₁ v₁ : List (Symbol T g.Nt),
        u =
            (z::List.join (List.map (· ++ [h]) (List.take m (List.map (List.map wrapSym) x)))) ++
              List.map wrapSym u₁ ∧
          List.get? x m =
              some (u₁ ++ r₀.inputL ++ [Symbol.nonterminal r₀.inputN] ++ r₀.inputR ++ v₁) ∧
            v =
    ...
    (hyp :
      ∃ x : List (List (Symbol T g.Nt)),
        (∀ xᵢ ∈ x, GrammarDerives g [Symbol.nonterminal g.initial] xᵢ) ∧
          α = [z] ++ List.join (List.map (· ++ [h]) (List.map (List.map wrapSym) x))) :
    (∃ x : List (List (Symbol T g.Nt)),
        (∀ xᵢ ∈ x, GrammarDerives g [Symbol.nonterminal g.initial] xᵢ) ∧
          α' = [z] ++ List.join (List.map (· ++ [h]) (List.map (List.map wrapSym) x))) ∨
      ∃ x : List (List (Symbol T g.Nt)),
...
private lemma case_2_match_rule {g : Grammar T} {r₀ : Grule T g.Nt}
    ...
    ∃ m : ℕ,
      ∃ u₁ v₁ : List (Symbol T g.Nt),
        u =
            (r::h::List.join (List.map (· ++ [h]) (List.take m (List.map (List.map wrapSym) x)))) ++
              List.map wrapSym u₁ ∧
          List.get? x m =
              some (u₁ ++ r₀.inputL ++ [Symbol.nonterminal r₀.inputN] ++ r₀.inputR ++ v₁) ∧
            v =
  ...
  · cases' x with x₀ L
    ...
    · right; right; right; right
      ...
      rw [u_nil, List.nil_append, v_eq, rin, List.nil_append, List.map_cons, List.map_cons,
        List.join, List.append_assoc, List.append_join_map_append, ← List.append_assoc] at aft 
      constructor
      · use
          List.map wrap_sym x₀ ++
...
private lemma star_case_3 {g : Grammar T} {α α' : List (Ns T g.Nt)}
  ...
  · rw [rin] at bef aft 
    ...
    · right; right; right
      ...
      have u_eq :
        ...
        case_3_u_eq_left_side bef
      have v_eq :
        v = List.join (List.map (· ++ [H]) (List.map (List.map wrap_sym) (L.reverse ++ [xₘ]))) :=
        by
        rw [u_eq] at bef 
        rw [gamma_nil_here, List.map_nil, List.append_nil] at bef 
        exact (List.append_left_cancel bef).symm
      ...
      · use
          List.map Symbol.terminal w.join ++ List.map Symbol.terminal β ++
              List.join (List.map (· ++ [H]) (List.map (List.map wrap_sym) L.reverse)) ++
            List.map wrap_sym xₘ
        rw [List.map_append, List.map_append, List.join_append, List.map_singleton,
          List.map_singleton, List.join_singleton, ← List.append_assoc, ← List.append_assoc]
        rfl
      constructor
      · intro contra
        rw [List.mem_append] at contra 
  ...
  · rcases h with ⟨m, u₁, v₁, u_eq, xm_eq, v_eq⟩
    ...
    · rw [aft]
      trace
        "./././Mathport/Syntax/Translate/Tactic/Builtin.lean:73:14: unsupported tactic `trim #[]"
      rw [List.map_append_append, List.map_append_append, List.join_append_append, ← List.map_take,
        ← List.map_drop, List.map_singleton, List.map_singleton, List.join_singleton,
