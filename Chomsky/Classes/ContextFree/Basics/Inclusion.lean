...
lemma CFG.toGeneral_well_defined (g : CFG T) :
  grammar_of_csg (csg_of_cfg g) = CFG.toGeneral g :=
by
  sorry

lemma grammar_of_csg_of_cfg :
  grammar_of_csg ∘ csg_of_cfg = @CFG.toGeneral T :=
by
  ext
  apply CFG.toGeneral_well_defined
...
by
  rw [←CFG.toGeneral_well_defined, cfLanguage_eq_csLanguage, csLanguage_eq_grammarLanguage]
...
by
  rintro ⟨g, eq_L⟩
  use csg_of_cfg g
  rw [←eq_L, cfLanguage_eq_csLanguage]
